...
public class ContextHolder implements ApplicationContextAware {
   ...
   */
  private static final ThreadLocal<HttpServletResponse> responseThreadLocal =
      new ThreadLocal<HttpServletResponse>();
  /**
   * 保存HttpServletRequest的线程局部变量.
   */
  private static final ThreadLocal<HttpServletRequest> requestThreadLocal =
      new ThreadLocal<HttpServletRequest>();

  /**
   * 用于在当前线程中存放当前线程操作采用的数据源名称
   */
  private static final ThreadLocal<String> currentDataSourceNameThreadLocal =
      new ThreadLocal<String>();

  /**
   * 存储当前线程当中开启的Hibernate Session，以当前session所在的dataSource name为key，在纯种结束时将所有未close的session执行close操作
   */
  private static final ThreadLocal<Map<String, Session>> hibernateSessionMapThreadLocal =
  ...
  public void setApplicationContext(ApplicationContext context) {
    ...
    if (StringUtils.isNotEmpty(tmpdir)) {
      ...
      if (!file.exists() && applicationContext instanceof WebApplicationContext) {
        BDF_TEMP_DIR = ((WebApplicationContext) applicationContext).getServletContext()
            .getRealPath(BDF_TEMP_DIR);
      }
  ...
  public static void setHttpRequestResponseHolder(HttpServletRequest request,
