...
public abstract class AbstractDao implements IDao {
  ...
  protected ParseResult parseCriteria(Criteria criteria, boolean useParameterName, String alias) {
    ...
    for (Criterion c : criteria.getCriterions()) {
      ...
      count++;
      parameterNameCount =
          buildCriterion(sb, c, valueMap, parameterNameCount, useParameterName, alias);
    }
    return result;
  }

  private int buildCriterion(StringBuffer sb, Criterion c, Map<String, Object> valueMap,
      int parameterNameCount, boolean useParameterName, String alias) {
    ...
    if (c instanceof Junction) {
      ...
      if (criterions != null) {
        ...
        for (Criterion criterion : criterions) {
          ...
          count++;
          parameterNameCount = this.buildCriterion(sb, criterion, valueMap, parameterNameCount,
              useParameterName, alias);
        }
        sb.append(" ) ");
      }
    }
